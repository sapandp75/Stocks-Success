...
export default function Scenarios({ ai, currentPrice }) {
  ...
  if (scenarios) {
    ...
    } else if (scenarios.worst && scenarios.best) {
      worst = scenarios.worst.target_price
      base = scenarios.base?.target_price
      best = scenarios.best.target_price
      weightedTarget = [scenarios.worst, scenarios.base, scenarios.best].filter(Boolean)
        .reduce((sum, s) => sum + (s.target_price || 0) * (s.probability || 0.33), 0)
    }
  }

  const scenarioRows = scenarios
    ? (Array.isArray(scenarios) ? scenarios : [scenarios.worst, scenarios.base, scenarios.best].filter(Boolean))
    : []
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {scenarioRows.map((s, i) => {
                    const name = s.name || s.scenario || ['Worst', 'Base', 'Best'][i] || `Scenario ${i + 1}`
