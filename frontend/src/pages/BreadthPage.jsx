...
function signalBadge(signal) {
  ...
  // Only structurally positive breadth states are green
  const bullish = ['BULLISH', 'STRONG', 'HEALTHY', 'ADVANCING']
  // Late-cycle / stretched / cautionary — amber not green
  const cautionary = ['OVERBOUGHT', 'LOW', 'COMPLACENT', 'RECOVERING', 'NEUTRAL', 'SLIGHTLY_ADVANCING', 'SLIGHTLY_DECLINING', 'MARGINAL', 'NORMAL', 'ELEVATED']
  const bearish = ['BEARISH', 'DEEPLY_BEARISH', 'POOR', 'DECLINING', 'OVERSOLD', 'EXTREME', 'HIGH', 'EXTREME_FEAR', 'PANIC_SELLING', 'FEAR', 'WEAKENING']
  let bg, color
  if (bullish.includes(signal)) { bg = '#dcfce7'; color = COLORS.green }
  else if (bearish.includes(signal)) { bg = '#fef2f2'; color = COLORS.red }
  else if (cautionary.includes(signal)) { bg = '#fef9c3'; color = COLORS.amber }
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-lg border p-4 mb-6 flex items-center gap-4 flex-wrap" style={{ borderColor: COLORS.border }}>
        ...
        <div className="flex gap-4 text-sm" style={{ color: COLORS.muted }}>
          ...
          <span>$NASI {mc.nasi?.value?.toLocaleString() ?? 'N/A'}</span>
      ...
      {/* McClellan — NYSE + Nasdaq Composite */}
      ...
      {/* Advance/Decline + Highs/Lows — NYSE + Nasdaq Composite */}
      ...
      {/* Participation — S&P 500 + NDX 100 + Bullish % */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <ValueCard label="$BPSPX — S&P 500 Bullish %" data={bp.spx != null ? { value: bp.spx, change: 0, signal: bp.spx >= 60 ? 'BULLISH' : bp.spx >= 40 ? 'NEUTRAL' : 'BEARISH' } : null} />
        <ValueCard label="$BPNDX — Nasdaq 100 Bullish %" data={bp.ndx != null ? { value: bp.ndx, change: 0, signal: bp.ndx >= 60 ? 'BULLISH' : bp.ndx >= 40 ? 'NEUTRAL' : 'BEARISH' } : null} />
      </div>
